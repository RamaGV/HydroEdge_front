...
const CultivoCard: React.FC<RecetaData> = ({ _id, nombre, totalDias = 112, dificultad = 'Difícil', resumen = 'Ajuste de pH, EC y temperatura para todas las fases' }) => {
  ...
  return (
    <div className={`rounded-lg shadow-lg transition-colors duration-300 mt-6
      bg-gradient-to-br from-gray-800 to-gray-700 p-4 shadow-xl 
      ${isDarkMode ? 'dark:bg-gray-800 dark:text-white' : 'bg-white text-gray-200'}`}>
      <Link href={`/recetaDashboard`} className="block p-4" onClick={() => handleSelectReceta(_id)}>
        ...
        <div className="flex flex-col text-center mt-6 justify-between items-center">
          <div className="flex flex-row w-full justify-between items-center">
            ...
            <div>
              <p className="font-semibold text-sm text-gray-500">Dificultad</p>
              <span className={`text-xl font-bold ${dificultad === 'Fácil' ? 'text-green-500' : dificultad === 'Media' ? 'text-yellow-500' : 'text-red-500'}`}>
                {dificultad}
              </span>
            </div>
          </div>
